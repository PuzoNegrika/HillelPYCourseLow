...
print(len(example[-2][-1]))
print()

# Списки можна складати
# Списки в Пайтон можна складати різними способами, залежно від того, що ви хочете отримати. Ось деякі з них:
# Використовувати операцію +, яка приймає два списки і повертає новий список, який є їх конкатенацією. Цей спосіб дозволяє об'єднати два списки в один, але не змінює початкові списки
first_list = [2, 4, 7]
second_list = [3, 5, 7]
my_list = first_list + second_list
print(my_list)  # [2, 4, 7, 3, 5, 7]

# Використовувати операцію += додає всі елементи другого списку в кінець першого. Цей спосіб дозволяє розширити список іншим списком, але змінює початковий список
first_list1 = [2, 4, 7]
second_list1 = [3, 5, 8]
first_list1 +=  second_list1
print(first_list1)  # [2, 4, 7, 3, 5, 8]

# Використовувати метод extend(). extend() - метод оновлює список, додаючи елементи до кінця початкового списку. Коли ви викликаєте цей метод, він перебирає аргументи і поміщає їх у список один за одним у хвостову частину (теж, що і додавання вище). Він приймає лише один параметр (об'єкт, що ітерується) і нічого не повертає:
lst = [3, 5, 7]
lst.extend([8, 4, 56]) # також в дужки можна поміщати і якісь змінні (типу lst, first_list etc)
print(lst) # [3, 5, 7, 8, 4, 56]

first_list1 = [2, 4, 7]
second_list1 = [3, 5, 8]
second_list2 = [3, 5, 10]
first_list1.extend(second_list1)
first_list1.extend(second_list2)
print(first_list1)
...
print(first_list) # [2, 4, 7, [3, 5, 7]]
print()

# Збільшення списку на ціле число
...
# Потрібно бути обережним при створенні нового списку, коли в якості елементів використовується тип даних, що змінюється
l = [[]] * 3
print(l)
...
print(example)
x = example[2:8] # с 2 елементу до 7 (бо 8 не враховується(тобто ДО якого ми обрізаємо список))
print(x)
x = example[2:8:2] # ['someText', True, None], переступає 1 аргумент через кожен наступний
print(x)
x = example[6:] # від 6 і до кінця
print(x)
x = example[::-1] # копія списку з елементами у зворотному напрямку
print(x)
